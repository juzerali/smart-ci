...
## Simplifications
All changes are tracked in respective files. Ideally only `*.tf` file changes should trigger the pipeline.

## Decision
Native `paths` filter vs `dorny/paths-filter`.

While native `paths` filter makes the script shorter, it lacks full flexibility for providing custom output based on various use cases. It is more extensible for future cases that can grow more complex.